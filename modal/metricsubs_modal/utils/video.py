...
def transcode_video_to_1080p(video_path: Path, output_path: Path):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"FFmpeg stderr: {e.stderr.decode() if e.stderr else 'No stderr'}")
            raise Exception(f"Failed to probe video file: {video_path}")
        ...
        # If already 1080p or smaller, just copy
        if original_height <= 1080:
            logger.info("Video is already 1080p or smaller, copying...")
            try:
                (
                    ffmpeg
                    .input(str(video_path))
                ...
                logger.error(f"FFmpeg copy error: {e}")
                logger.error(f"FFmpeg stderr: {e.stderr.decode() if e.stderr else 'No stderr'}")
                raise
            return
