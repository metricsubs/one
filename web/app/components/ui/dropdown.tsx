...
const dropdownSectionStyles = tv({
    slots: {
        ...
        header: 'col-span-full px-3.5 py-2 font-medium text-muted-fg text-sm/6 sm:px-3 sm:py-1.5 sm:text-xs/6',
    },
});
            ...
            {composeRenderProps(children, (children, { isSelected }) => (
                <>
                    ...
                    {isSelected && (
                        <FaCheck
                            className={cn(
                                'ml-2 text-muted-fg group-hover:text-muted'
                            )}
...
const DropdownLabel = ({ className, ref, ...props }: DropdownLabelProps) => (
    <Text slot="label" ref={ref} className={twMerge(className)} {...props} />
);
